...
def check_and_create_site(ngwaf_user_email, ngwaf_token, corp_name, site_name):
    ...
    if check_response.status_code == 200:
        ...
        try:
            ...
            if response_json.get("message") in ["Site not found", "Invalid resource request"]:
...
def process_single_site(ngwaf_user_email, ngwaf_token, fastly_token, corp_name, site_name, fastly_sid, activate_version, percent_enabled, dynamic_backend, premier, show_response_headers=False):
    # Check if the site exists and create it if necessary
    ...
    # Check if the NG WAF object exists and create it if necessary
    ngwaf_object_exists = check_ngwaf_object_exists(ngwaf_user_email, ngwaf_token, fastly_token, corp_name, site_name)
    skip_wait = False
    if not ngwaf_object_exists:
        ...
        if create_response.status_code != 200:
            ...
            return
        skip_wait = False  # Wait for the new edge security object to be configured
    else:
        print(f"NG WAF object exists for {site_name}, skipping edge object creation.")
        skip_wait = True

    # Add the corp to the appropriate groups if flags are passed
    if dynamic_backend:
    ...
    # Map the site to the Fastly service
